...
public class GamePlayLoading : MonoBehaviourPunCallbacks
{
    ...
    public void Prepare()
    {
        ...
        m_Master.text = PhotonNetwork.IsMasterClient ? Core.state.playerName : Core.networkManager.member.mbr_id;
        m_Player.text = PhotonNetwork.MasterClient.NickName;
        m_MasterBar.value = 0;
        m_PlayerBar.value = 0;
    ...
    IEnumerator Loading(UnityAction done)
    {
        ...
        while (value < 1)
        {
            ...
            {
                photonView.RPC("UpdateLoadingProgress", RpcTarget.All, 0.5f);
                isUpdated = true;
            }
    ...
    void OnCreateCharacters(UnityAction done = null)
    {
		...
		Transform tr = PhotonNetwork.Instantiate(XSettings.chracterPath + name, createPoints[playerIndex].position, createPoints[playerIndex].rotation, 0).transform;
