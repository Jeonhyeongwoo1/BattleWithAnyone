...
public class RoomMenu : MonoBehaviourPunCallbacks
{
	...
	public void SearchRoom(string roomName)
	{
		...
		foreach (KeyValuePair<string, RoomInfo> roomInfo in m_CachedRoomList)
		{
			...
			if (o)
			{
				string roomManager = (string)roomInfo.Value.CustomProperties["RoomManager"];
				string mapTitle = (string)roomInfo.Value.CustomProperties["Map"];
				Room room = Instantiate<Room>(roomPrefab, Vector3.zero, Quaternion.identity, m_SearchedRoomContent);
				room.SetRoomInfo(roomInfo.Key, roomManager, mapTitle);
				room.name = roomInfo.Key;
				count++;
			}
		}
	...
	void RoomListUpdate(List<RoomInfo> roomList)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			else
			{
				...
				Room room = Instantiate<Room>(roomPrefab, Vector3.zero, Quaternion.identity, m_RoomContent);
	...
	void RefreshRoom()
	{
		...
		StartCoroutine(RefresingRoom());
	}

	IEnumerator RefresingRoom()
	{
		m_IsRefreshing = true;
